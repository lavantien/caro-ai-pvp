    ...
    )
    {
        ...
        while (totalMoves < maxMoves && !game.IsGameOver)
        {
            ...
            try
            {
                ...
                var (depthAchieved, nodesSearched, nodesPerSecond, tableHitRate, _, vcfDepthAchieved, vcfNodesSearched, threadCount, parallelDiagnostics, masterTTPercent, helperAvgDepth, allocatedTimeMs, bookUsed, moveType, _, fmcPercent, ebf) = currentAI.GetSearchStatistics();
                ...
                var stats = new MoveStats(depthAchieved, nodesSearched, nodesPerSecond, tableHitRate, ponderingActive, vcfDepthAchieved, vcfNodesSearched, threadCount, parallelDiagnostics, moveTimeMs, masterTTPercent, helperAvgDepth, allocatedTimeMs, ponderNodes, ponderNps, ponderDepth, bookUsed, moveType, ebf, fmcPercent);
