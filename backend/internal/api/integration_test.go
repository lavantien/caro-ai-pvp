...
func decodeGameID(t *testing.T, body []byte) string {
	...
	id, ok := resp["game_id"].(string)
	require.True(t, ok, "response should have game_id")
	return id
}
...
func TestIntegrationFullGameFlow(t *testing.T) {
	...
	resp, err := http.Post(srv.URL+"/api/games", "application/json", strings.NewReader(
		`{"time_control":"3+2","game_mode":"pvp"}`,
	))
	...
	// 2. Make moves to create a winning line: Red plays (3,0)-(7,0), Blue plays (3,1)-(6,1)
	moves := []struct {
		x, y int
	}{
		{3, 0}, {3, 1}, // Red(3,0), Blue(3,1)
		{4, 0}, {4, 1}, // Red(4,0), Blue(4,1)
		{5, 0}, {5, 1}, // Red(5,0), Blue(5,1)
		{6, 0}, {6, 1}, // Red(6,0), Blue(6,1)
		{7, 0},         // Red(7,0) -> Red wins with 5 in a row
	}
	for _, m := range moves {
		moveResp, err := http.Post(srv.URL+"/api/games/"+gameID+"/moves", "application/json",
	...
	getResp, err := http.Get(srv.URL + "/api/games/" + gameID)
	require.NoError(t, err)
	readBody(t, getResp, &body)
	getResp.Body.Close()
	state = decodeState(t, body)
	assert.True(t, state["is_game_over"].(bool))

	// 4. Delete game
	delResp, err := http.NewRequest(http.MethodDelete, srv.URL+"/api/games/"+gameID, nil)
	require.NoError(t, err)
	client := &http.Client{}
	...
	// 5. Verify deleted
	getResp2, err := http.Get(srv.URL + "/api/games/" + gameID)
	require.NoError(t, err)
	getResp2.Body.Close()
...
func TestIntegrationCreateUndoRedo(t *testing.T) {
	...
	// Move
	moveResp, _ := http.Post(srv.URL+"/api/games/"+gameID+"/moves", "application/json",
	...
	// Undo
	undoResp, _ := http.Post(srv.URL+"/api/games/"+gameID+"/undo", "application/json", nil)
...
func TestIntegrationConcurrentGames(t *testing.T) {
	...
	for i := range 10 {
		...
		go func(n int) {
			defer wg.Done()
			resp, err := http.Post(srv.URL+"/api/games", "application/json",
				strings.NewReader(`{"time_control":"1+0"}`))
			if err != nil {
