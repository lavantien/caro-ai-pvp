...
public class NodeCountingTests
{
    ...
    public async Task ParallelSearch_RealNodeCountShouldVaryBetweenMoves()
    {
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            var (_, nodesSearched, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) = ai.GetSearchStatistics();
    ...
    public void ParallelSearch_SamePositionSameDepth_NodesShouldBeConsistent()
    {
        ...
        var (_, nodes1, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) = ai.GetSearchStatistics();

        // Clear AI state for second search
        ai.ClearAllState();

        var (x2, y2) = ai.GetBestMove(board, Player.Red, AIDifficulty.Hard);
        var (_, nodes2, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) = ai.GetSearchStatistics();
    ...
    public void ParallelSearch_DifferentDepths_NodesShouldIncreaseWithDepth()
    {
        ...
        var (_, nodesLow, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) = ai.GetSearchStatistics();

        ai.ClearAllState();

        var (x2, y2) = ai.GetBestMove(board, Player.Red, AIDifficulty.Hard);
        var (_, nodesHigh, _, _, _, _, _, _, _, _, _, _, _, _, _, _, _) = ai.GetSearchStatistics();
